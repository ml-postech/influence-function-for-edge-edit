...
#from torch.func import linearize, functional_call, module_to_pytree, tree_map
...
class BaseInfluenceModule(abc.ABC):
    ...
    def stest(self, test_idxs=None, grad_eval=None) -> torch.Tensor:
        ...
        if self.full_batch:
            if grad_eval is None:
                assert test_idxs is not None
                self.saved_stest, norm = self.inverse_hvp_on_graph(self.test_loss_grad(test_idxs))
            else:
                assert test_idxs is None
                self.saved_stest, norm = self.inverse_hvp_on_graph(grad_eval)
        else:
            self.saved_stest = self.inverse_hvp(self.test_loss_grad(test_idxs))
                
        return self.saved_stest, norm
    ...
    def influences(
        ...
        """
        print('Calculating inverse-Hessian product...')
        stest = self.stest(test_idxs) if (stest is None) else stest.to(self.device)

        scores = []


        print('Calculating influence score...')
        if self.full_batch:
            params = self._model_params(with_names=False)
            flat_params = self._flatten_params_like(params)
            loss_fn = self.objective.indiv_train_loss
            for idx in tqdm(train_idxs):
                loss = loss_fn(model=self.model, params=flat_params, batch=self.graph, idx=idx)
                grad_z = self._flatten_params_like(torch.autograd.grad(loss, params))
                s = grad_z @ stest
                scores.append(s)
            return torch.tensor(scores) / self.graph.train_mask.sum()
        else:
    ...
    def _reshape_like_params_dict(self, vec):
        pointer = 0
        param_dict = {}
        self.param_info = [(name, p.detach()) for name, p in self.model.model.named_parameters()]
        for name, ref in self.param_info:          # 예: ('layer1.weight', tensor_ref)
            n = ref.numel()
            param_dict[name] = vec[pointer : pointer + n].view_as(ref)
            pointer += n
        return param_dict
    ...
    def _loss_grad(self, idxs, train):
        if self.full_batch:
            ...
            flat_params = self._flatten_params_like(params)
            loss_fn = self.objective.train_loss if train else self.objective.test_loss
            loss = loss_fn(model=self.model, params=flat_params, batch=self.graph)
            grad = self._flatten_params_like(torch.autograd.grad(loss, params))
            return grad
        else:
            grad = 0.0
    ...
    def _hvp_graph(self, graph, flat_params, vec, gnh):
        def f(theta_):
            self._model_reinsert_params(self._reshape_like_params(theta_))
            return self.objective.train_loss(self.model, theta_, graph)
        
        def hvp_f(theta_):
            self._model_reinsert_params(self._reshape_like_params(theta_))
            return self.objective.train_loss_without_reg(self.model, graph)

        def out_f(theta_):
            self._model_reinsert_params(self._reshape_like_params(theta_))
            return self.objective.train_outputs(self.model, graph)
        

        def loss_f(out_):
            return self.objective.train_loss_on_outputs(out_, graph)
        ...
        if gnh:
            ...
            if self.vjp_func is None:
                _, self.vjp_func = torch.func.vjp(out_f, flat_params)
            gnhvp_batch = self.vjp_func(hjvp)[0]

            #print(torch.allclose(gnhvp_batch, nvjp))
            return gnhvp_batch + torch.autograd.functional.hvp(reg_f, flat_params, v=vec)[1]
        else:
            return torch.autograd.functional.hvp(hvp_f, flat_params, v=vec)[1]
